...
class MotifFinding:
    ...
    def heuristicConsensus(self):
        # Procura as posicoes para o motif nas 2 primeiras seqs
        # fazer uma procura exaustiva nas 2 primeiras seqs

        # forma naive:
        mf = MotifFinding(self.motifSize,self.seqs[:2])
        s = mf.exhaustiveSearch()
        # avaliar a melhor posicao para cada uma das seq seguintes
        # uma a uma, guardadndo a melhor posicao (maximiza o score)
    ...
    def heuristicStochastic (self):
        ...
        while improve:
            ...
            motif.createPWM()
            #passo 3
            # avalia a melhor posicao inicial para cada seq com base no perfil
            for i in range(len(self.seqs)):
