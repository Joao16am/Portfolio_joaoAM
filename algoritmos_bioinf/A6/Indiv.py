...
class IndivReal(Indiv):
    ...
    def initRandom(self, size):
        self.genes = []
        for _ in range(size):
            self.genes.append(uniform(self.lb, self.ub))  #(uniform())Serve para que qualquer valor tenha
                                                      # a mesma probabilidade ttem de sair
                                            # Pode usar-se random, mas para tal é necessário definir
                                            # o __init__ e estabelecer um lowerbound e um upperbound
