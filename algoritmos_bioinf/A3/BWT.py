...
class BWT:
    ...
    def build_bwt(self, text, buildsufarray = False):
        ...
        for i in range(len(text)):
            ls.append(text[i:] + text[:i]) # construcao das rotacoes
        ls.sort() # organizacao
        res = " "
        for i in range(len(text)):
            res += ls[i][len(text) - 1] #buscar a ultima coluna da matriz BW
    ...
    def last_to_first(self):
        ...
        for i in range(len(firstcol)):
            c = self.bwt[i]
            ocs = self.bwt[:i].count(c) + 1 #Indepede repeticoes pelo bwt[:i]
