...
class OverlapGraph(MyGraph):
    ...
    def __init__(self, frags, reps):
        MyGraph.__init__(self, {})
        if not reps: self.create_overlap_graph(frags)
        else: self.create_overlap_graph_with_reps(frags)
        self.reps = reps
    ...
    def create_overlap_graph(self, frags, rep = False):
        ## add vertices
        for frag in frags:
            self.add_vertex(frag)

        ## add edges
        for frag in frags:
            suf = suffix(frag)
            for frag2 in frags:
                if prefix(frag2) == suf: self.add_edge(frag, frag2)
